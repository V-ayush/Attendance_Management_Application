...
public class CourseController {
	...
	public ResponseEntity<CourseEntity> addCourse(@Valid @RequestBody CourseEntity entity)
		{
				return new ResponseEntity<CourseEntity>(courseService.addCourse(entity),HttpStatus.CREATED);
		}
	...
	public ResponseEntity<String>deleteByCourseId(@Valid @PathVariable int courseId) 
	...
	@GetMapping(path="/getCourseByName/{courseName}") 
	  public ResponseEntity<List<CourseEntity>> findCourseByName(@PathVariable String courseName) 
			  throws RecordNotFoundException
		{
				return new ResponseEntity<List<CourseEntity>>(courseService.findCourseByName(courseName),HttpStatus.FOUND);
		}
